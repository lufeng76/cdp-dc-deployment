...
Step1.  先挂载操作系统iso文件，然后拷贝到/var/www/html目录
....
# 登录终端服务器
mkdir /media/DVD1
...
Step2.  将Cloudera Manager安装需要的6个rpm包以及一个asc文件转移到/var/www/html目录 +
...
Step3.  将安装postgresql-10需要的rpm包转移到/var/www/html目录 +
然后执行createrepo命令生成rpm元数据。
....
...
Step4.  将CDP安装需要的三个文件转移到/var/www/html目录
....
mv cdh7.1/ /var/www/html/
....

Step5.  其他的rpm包采取类似于CM的方法，parcel包采取类似于CDP的方法。

Step6.  验证浏览器http://<IP_ADDRESS>/cm7.1能否正常访问  
